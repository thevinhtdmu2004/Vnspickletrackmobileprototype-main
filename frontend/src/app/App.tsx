...
import { PeopleOperationsScreen } from './components/PeopleOperationsScreen';
import { CoachOperationsScheduleScreen } from './components/CoachOperationsScheduleScreen';
import { CoachReconciliationScreen } from './components/CoachReconciliationScreen';
import { ExtendedOperationsScreen } from './components/ExtendedOperationsScreen';
import { ExperienceOperationsScreen } from './components/ExperienceOperationsScreen';
import { OwnerReportsScreen } from './components/OwnerReportsScreen';
import { CourtManagementScreen } from './components/CourtManagementScreen';
import { BookingConflictsScreen } from './components/BookingConflictsScreen';
import { SystemAlertsScreen } from './components/SystemAlertsScreen';
...
/* ── Newly copied Member portal components & services ── */
import { MemberCourtBookingScreen } from './components/MemberCourtBookingScreen';
import { MemberCourtBookingHistoryScreen } from './components/MemberCourtBookingHistoryScreen';
...
import { MemberMembershipRequestHistoryScreen } from './components/MemberMembershipRequestHistoryScreen';
import { MemberEquipmentRentalScreen } from './components/MemberEquipmentRentalScreen';
import { MemberEquipmentRentalHistoryScreen } from './components/MemberEquipmentRentalHistoryScreen';
import { MemberAttendanceHistoryScreen } from './components/MemberAttendanceHistoryScreen';
import { getBookingCourtSummary, type CourtBooking } from './components/memberCourtBooking';
import { createEquipmentRentalOrder } from './services/memberEquipmentRentalService';
import { createMembershipRequest } from './services/memberMembershipService';
...
function BookingSuccessDialog({ booking, onClose }: BookingSuccessDialogProps) {
  const createdAt = new Date(booking.createdAtISO);
  const courtSummary = getBookingCourtSummary(booking);
  const requestCode = `BK-${createdAt.getHours().toString().padStart(2, '0')}${createdAt.getMinutes().toString().padStart(2, '0')}-${booking.slots.length}`;
  const slotSummary = booking.slots.map(slot => `${slot.timeStart} - ${slot.timeEnd}`).join(' · ');
        ...
        <div className="px-5 pt-4 pb-5">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 800, letterSpacing: '0.05em' }}>
                  GIỮ CHỖ SÂN THÀNH CÔNG
                </p>
                <p style={{ fontSize: 20, fontWeight: 900, color: '#1F2933', marginTop: 2 }}>
                  {courtSummary}
                </p>
              </div>
            </div>
          ...
          <p style={{ fontSize: 13, color: '#6B7280', lineHeight: 1.55, marginBottom: 14 }}>
            Hệ thống đã ghi nhận giữ chỗ sân cho hội viên. Popup này giúp xác nhận nhanh chi tiết trước khi bạn tiếp tục thao tác.
          </p>

          <div className="space-y-2.5 mb-4">
            <div className="rounded-2xl px-3.5 py-3" style={{ background: 'rgba(14,124,123,0.06)' }}>
              <div className="flex items-center gap-2 mb-1">
                <CalendarDays style={{ width: 14, height: 14, color: '#0E7C7B' }} />
                <span style={{ fontSize: 11, fontWeight: 800, color: '#6B7280' }}>Ngày sử dụng</span>
              </div>
              <p style={{ fontSize: 14, fontWeight: 900, color: '#1F2933' }}>{booking.dateLabel}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="rounded-2xl px-3.5 py-3" style={{ background: 'rgba(14,124,123,0.06)' }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span style={{ fontSize: 11, fontWeight: 800, color: '#6B7280' }}>Khung giờ</span>
                </div>
                <p style={{ fontSize: 14, fontWeight: 900, color: '#1F2933' }}>
                  {booking.slotCount > 1 ? `${booking.slotCount} khung giờ` : `${booking.timeStart} - ${booking.timeEnd}`}
                </p>
                {booking.slotCount > 1 && (
              ...
              <div className="rounded-2xl px-3.5 py-3" style={{ background: 'rgba(244,162,97,0.08)' }}>
                <div className="flex items-center gap-2 mb-1">
                  <Bell style={{ width: 14, height: 14, color: '#E8832A' }} />
                  <span style={{ fontSize: 11, fontWeight: 800, color: '#6B7280' }}>Nhắc sân</span>
                </div>
                <p style={{ fontSize: 14, fontWeight: 900, color: '#E8832A' }}>
                  Trước {booking.reminderMinutes} phút
                </p>
              </div>
            </div>
            ...
            <div className="rounded-2xl px-3.5 py-3" style={{ background: 'rgba(14,124,123,0.06)' }}>
              <div className="flex items-center gap-2 mb-1">
                <MapPin style={{ width: 14, height: 14, color: '#0E7C7B' }} />
                <span style={{ fontSize: 11, fontWeight: 800, color: '#6B7280' }}>Số người dự kiến</span>
              </div>
              <p style={{ fontSize: 14, fontWeight: 900, color: '#1F2933' }}>
            ...
            <div className="rounded-2xl px-3.5 py-3" style={{ background: 'rgba(0,0,0,0.03)' }}>
              <div className="flex items-center gap-2 mb-1">
                <MapPin style={{ width: 14, height: 14, color: '#0E7C7B' }} />
                <span style={{ fontSize: 11, fontWeight: 800, color: '#6B7280' }}>Mã giữ chỗ</span>
              </div>
              <p style={{ fontSize: 14, fontWeight: 900, color: '#1F2933' }}>{requestCode}</p>
              <p style={{ fontSize: 11, color: '#6B7280', lineHeight: 1.45, marginTop: 4 }}>
                Hội viên có thể xem lại ở trang chủ hoặc vào màn giữ chỗ sân để đổi giờ và hủy khi cần.
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="w-full py-3.5 rounded-2xl active:opacity-85 transition-opacity"
...
export default function App() {
  ...
  const [bookingSuccess, setBookingSuccess] = useState<CourtBooking | null>(null);
  ...
  function showBookingSuccess(booking: CourtBooking, cb: () => void) {
    setBookingSuccess(booking);
  }

  function onBookingSuccessClose() {
  ...
  function renderScreen() {
    ...
    switch (currentScreen as string) {
      ...
      case 'login':
        return (
          <LoginScreen onLogin={(r, activePkg) => {
            setRole(r);
            if (r === 'member') {
              setHasActivePackage(activePkg);
            }
      ...
      case 'member-court-booking':
        return (
          <MemberCourtBookingScreen
            onBack={goBack}
            onBooked={(booking) => showBookingSuccess(booking, () => {})}
            onRescheduled={() => showSuccess('Đã đổi khung giờ sân thành công!', () => {})}
            onCancelled={() => showSuccess('Đã hủy đặt sân thành công.', () => {})}
            onViewHistory={() => navigate('member-court-booking-history')}
          />
        );

      case 'member-court-booking-history':
        return <MemberCourtBookingHistoryScreen onBack={goBack} />;

      case 'member-equipment-rental':
        return (
          <MemberEquipmentRentalScreen
            onBack={goBack}
            onViewHistory={() => navigate('member-equipment-rental-history')}
            onConfirm={async (selections) => {
              await createEquipmentRentalOrder(selections);
              showSuccess('Đã ghi nhận yêu cầu thuê đồ tại sân!', () => navigate('member-equipment-rental-history', true));
            }}
          />
        );

      case 'member-equipment-rental-history':
        return <MemberEquipmentRentalHistoryScreen onBack={goBack} />;
      ...
      case 'member-membership-overview':
        return (
          <MemberMembershipOverviewScreen
            ...
            onViewHistory={() => navigate('member-payment-history')}
            onViewRequestHistory={() => navigate('member-membership-request-history')}
          />
        );
      ...
      case 'member-membership-registration':
        return (
          <MemberMembershipRegistrationScreen
            ...
            onViewHistory={() => navigate('member-payment-history')}
            onViewRequestHistory={() => navigate('member-membership-request-history')}
            onSubmit={async ({ action, plan, note }) => {
              ...
              await createMembershipRequest({
                membershipPlanId: plan.code,
                requestType: requestTypeMap[action],
                note: note?.trim() || undefined,
              });

              showSuccess(
                action === 'renew'
                  ? 'Đã gửi yêu cầu gia hạn gói hội viên! Admin sẽ xác nhận sớm.'
                  : action === 'upgrade'
                    ? 'Đã gửi yêu cầu nâng hạng gói hội viên! Admin sẽ xác nhận sớm.'
                    : action === 'downgrade'
                      ? 'Đã gửi yêu cầu chuyển gói hội viên! Admin sẽ xác nhận sớm.'
                  : 'Đã gửi yêu cầu đăng ký gói hội viên! Admin sẽ xác nhận sớm.',
                () => navigate('member-membership-request-history', true),
              );
            }}
          />
        );

      case 'member-membership-request-history':
        return (
          <MemberMembershipRequestHistoryScreen
            onBack={goBack}
            onViewPaymentHistory={() => navigate('member-payment-history')}
            onCreateRequest={() => navigate('member-membership-registration')}
            onViewOverview={() => navigate('member-membership-overview')}
          />
        );
        ...
        >
          ...
          {showMemberNav && (
            <MemberBottomNavigation
              ...
              onTabChange={(tab) => {
                if (tab === 'member-package' && !hasActivePackage) {
                  switchTab('member-membership-overview');
                } else {
                  switchTab(tab as Screen);
                }
              }}
          ...
          {bookingSuccess && (
            <BookingSuccessDialog booking={bookingSuccess} onClose={onBookingSuccessClose} />
          )}
