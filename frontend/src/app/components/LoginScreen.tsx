...
const DEMO_ACCOUNTS: {
  ...
  {
    role: 'member', label: 'Học viên',   user: 'student', phone: '0908888111', pin: '333333',
    color: '#815AD5', bg: 'rgba(129,90,213,0.09)',  border: 'rgba(129,90,213,0.25)',
    Icon: Award,      desc: 'Đã đăng ký gói học',
    hasActivePackage: true,
  },
  {
    role: 'member', label: 'Hội viên thường', user: 'member', phone: '0908888888', pin: '222222',
    color: '#E28743', bg: 'rgba(226,135,67,0.09)',  border: 'rgba(226,135,67,0.25)',
    Icon: User,       desc: 'Chưa đăng ký gói',
    hasActivePackage: false,
  },
];

/* Validate credentials */
function validateLogin(account: string, pin: string): { role: Role; hasActivePackage: boolean } | null {
  const a = account.trim().toLowerCase();
  for (const demo of DEMO_ACCOUNTS) {
    if ((a === demo.user || a === demo.phone) && pin === demo.pin) {
      return { role: demo.role, hasActivePackage: demo.hasActivePackage ?? false };
    }
  }
  return null;
}
...
/* ══════════════════════════════════════════════════════
   COMPONENT
══════════════════════════════════════════════════════ */
export function LoginScreen({ onLogin }: { onLogin: (role: Role, hasActivePackage: boolean) => void }) {
  const [account,   setAccount]   = useState('student');
  const [pin,       setPin]       = useState('333333');
  const [showPin,   setShowPin]   = useState(false);
  const [error,     setError]     = useState('');
  const [loading,   setLoading]   = useState(false);
  const [quickRole, setQuickRole] = useState<string | null>('student');   // quick-fill
  ...
  function handleLogin() {
    ...
    setLoading(true);
    setTimeout(() => {
      const result = validateLogin(account, pin);
      if (result) {
        onLogin(result.role, result.hasActivePackage);
      } else {
            ...
            <div className="flex items-center gap-2 mb-3">
             ...
             <div className="space-y-2">
              {DEMO_ACCOUNTS.map(demo => {
                const isActive = quickRole === demo.user;
                return (
                  <button
                    key={demo.user}
