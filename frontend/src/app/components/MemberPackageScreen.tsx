...
import {
  ...
  BadgeCheck, CalendarDays, Clock3, Star, Wallet, ShieldCheck
} from 'lucide-react';
import {
  CURRENT_MEMBERSHIP,
...
export function MemberPackageScreen({ onNavigate, onRenew, hasActivePackage = true }: MemberPackageScreenProps) {
  const [activeTab, setActiveTab] = useState<'student' | 'membership' | 'history'>('student');
    ...
    <div className="relative flex flex-col min-h-screen" style={{ background: '#F0F4F5' }}>
      ...
      >
        ...
        <div className="relative px-5 pt-14 pb-4">
          ...
          <h1 style={{ fontSize:24, fontWeight:900, color:'white', letterSpacing:'-0.5px', marginTop:2 }}>
            {activeTab === 'student' ? 'Gói học viên' : activeTab === 'membership' ? 'Gói hội viên' : 'Hóa đơn thanh toán'}
          </h1>
          ...
          <div className="flex bg-black/20 rounded-xl p-1 mt-3.5">
            <button
              ...
              className={`flex-1 py-2 text-center text-xs font-bold rounded-lg transition-all ${
                activeTab === 'student' ? 'bg-white text-teal-900 shadow' : 'text-white/75 hover:text-white'
              }`}
            >
              Gói học viên
            </button>
            <button
              ...
              className={`flex-1 py-2 text-center text-xs font-bold rounded-lg transition-all ${
                activeTab === 'membership' ? 'bg-white text-teal-900 shadow' : 'text-white/75 hover:text-white'
              }`}
            >
              Gói hội viên
            </button>
      ...
      <div className="flex-1 overflow-y-auto pb-28">
        <div className="px-4 pt-4 space-y-4">
          ...
          {activeTab === 'student' && (
            /* ========================================================
               TAB: STUDENT (CURRENT CLASS SESSIONS PACKAGE)
               ======================================================== */
            <>
              {hasActivePackage ? (
                <>
          ...
          {activeTab === 'membership' && (
            /* ========================================================
               TAB: MEMBERSHIP (CURRENT MEMBERSHIP PACKAGE CARD & DETAILS)
               ======================================================== */
            <div className="space-y-4">
                      ...
                      <div className="mt-5">
                        ...
                        <div className="h-3 overflow-hidden rounded-full" style={{ background: 'rgba(255,255,255,0.18)' }}>
                          <div
                            ...
                            style={{ width: `${membershipProgress * 100}%`, background: 'rgba(255,255,255,0.92)' }}
                          />
                        </div>
                      </div>
                    </>
              ...
              <div className="bg-white rounded-3xl p-4 border border-teal-600/10 shadow-sm">
                ...
                <button
                  onClick={() => onNavigate?.('member-membership-registration')}
                  className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl active:scale-98 transition-all"
                  style={{
                    background: 'linear-gradient(135deg,#0E7C7B 0%,#2A9D8F 100%)',
