    ...
    >
      <div className="flex">
        {TABS.filter(tab => tab.id !== 'member-schedule' || hasActivePackage).map(tab => {
          const active = currentTab === tab.id || (tab.id === 'member-package' && currentTab === 'member-membership-overview');
          const Icon   = tab.icon;
          const label  = tab.id === 'member-package'
            ? (hasActivePackage ? 'Gói của tôi' : 'Gói hội viên')
            : tab.label;
